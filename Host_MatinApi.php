...
//{ include_once './IndexFileMatab/conect.php';

// $Data=array();

// $Qury = mysqli_query($conn,"SELECT * FROM `patients`");
...
// //$qury=mysqli_query($conn," insert INTO patient_visit (Height) VALUES(1244) ");
...
// if(isset($UserName)){
//     $qury=mysqli_query($conn," insert INTO patient_visit (Height) VALUES('$UserName') ");
//     if(!$qury){
...
include_once './MatabIndexFile/IndexFileMatab/conect.php';
...
class Matin {
    ...
    public static  function Matin_API_DB_Json($MatinQury,$DB_Conection,$Setting = [],$CustomeOperation=[],$CustomeMatch = []){
        ...
        if (preg_match($pattern, $MatinQury, $matches)) {
            ...
            if( isset($Setting [$tableName]) and $Setting [$tableName]== 'primery' or 'match' and !isset($_COOKIE[$tableName]) or $_COOKIE[$tableName]== '' ){
...
Matin::Matin_API_DB_Json(true,$conn,['visit'=>'match','patients'=>'close'],['visit' => ['line' => ['id'=>'close','UserName'=>'open','UserPassword'=> 'close']]],[ 'visit' => [ 'open' => [ 'visit' , 'patients' ] ] ]);
// Wolf::Wolf_API_DB("Defult_Api",$conn);


//$Wolf = new Wolf;

//Wolf::Wolf_Php('<div>></div> ');
